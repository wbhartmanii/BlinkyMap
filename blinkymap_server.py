...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [BlinkyMap] %(levelname)s %(message)s")
...
def _build_e131_packet(universe: int, channel_data: bytes) -> bytes:
    slots = len(channel_data)
    dmp_pdu_length = 11 + slots
    dmp = (
        struct.pack("!H", 0x7000 | dmp_pdu_length) + b"\x02\xa1"
        + struct.pack("!HHH", 0x0000, 0x0001, slots) + channel_data
    )
    source_name = b"BlinkyMap\x00" + b"\x00" * (64 - len("BlinkyMap\x00"))
    framing_pdu_length = 77 + len(dmp)
    framing = (
        struct.pack("!H", 0x7000 | framing_pdu_length) + b"\x00\x00\x00\x04"
        + source_name + struct.pack("!BBHB", 100, 0, universe, 0) + b"\x00" + dmp
    )
    root_pdu_length = 22 + len(framing)
    return (
        b"\x00\x10\x00\x00ASC-E1.17\x00\x00\x00"
        + struct.pack("!H", 0x7000 | root_pdu_length) + b"\x00\x00\x00\x04" + _CID + framing
    )


class E131Sender:
    def __init__(self, host: str, universe: int):
        self.host = host; self.universe = universe
        self._sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self._sock.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
    def send(self, channel_data: bytes):
        self._sock.sendto(_build_e131_packet(self.universe, channel_data), (self.host, 5568))
...
def _look_at_R(eye: np.ndarray, target: np.ndarray) -> np.ndarray:
    ...
    if np.linalg.norm(x) < 1e-6:
        up = np.array([0.0, 0.0, 1.0]); x = np.cross(z, up)
...
def _projection_matrix(sess: SessionConfig) -> np.ndarray:
    ...
    eye = np.array([sess.distance_m * math.sin(rad), sess.height_m, sess.distance_m * math.cos(rad)])
    R = _look_at_R(eye, np.array([0.0, sess.height_m * 0.5, 0.0]))
    return K @ np.hstack([R, (-R @ eye).reshape(3, 1)])


def _triangulate_pair(P1, pt1, P2, pt2):
    A = np.array([pt1[1]*P1[2]-P1[1], P1[0]-pt1[0]*P1[2], pt2[1]*P2[2]-P2[1], P2[0]-pt2[0]*P2[2]])
    _, _, Vt = np.linalg.svd(A)
    X = Vt[-1]
    if abs(X[3]) < 1e-10: return None
    X = X[:3] / X[3]
    if (P1[2] @ np.append(X, 1.0)) < 0 or (P2[2] @ np.append(X, 1.0)) < 0: return None
    return X


def _reprojection_error(P, X, pt):
    proj = P @ np.append(X, 1.0); proj /= proj[2]
    return float(np.hypot(proj[0]-pt[0], proj[1]-pt[1]))


def _angular_spread(projections):
    if len(projections) < 2: return 0.0
...
class PixelResult:
    ...
    def grade(self) -> str:
        ...
        if self.confidence >= 0.40: return "medium"
        if self.position is not None: return "low"
        return "unseen"
...
class BlinkyModel:
    ...
    def triangulate(self):
        if not self.sessions or self.pixel_count == 0: return
        proj = {sid: (_projection_matrix(sc), sc) for sid, (sc, _) in self.sessions.items()}
        results = {}
        for idx in range(self.pixel_count):
            obs = [(sid, detections[idx]) for sid, (_, detections) in self.sessions.items() if idx in detections]
            pr = PixelResult(index=idx, sessions_detected=[s for s, _ in obs])
            if len(obs) >= 2:
                candidates, reproj_errors, paired = [], [], []
                for i in range(len(obs)):
                    for j in range(i+1, len(obs)):
                        sid_a, det_a = obs[i]; sid_b, det_b = obs[j]
                        P_a, sc_a = proj[sid_a]; P_b, sc_b = proj[sid_b]
                        X = _triangulate_pair(P_a, (det_a.cx, det_a.cy), P_b, (det_b.cx, det_b.cy))
                        if X is not None:
                            candidates.append(X)
                            reproj_errors.append((_reprojection_error(P_a, X, (det_a.cx, det_a.cy)) +
                                                   _reprojection_error(P_b, X, (det_b.cx, det_b.cy))) / 2.0)
                            for item in [(P_a, sc_a), (P_b, sc_b)]:
                                if item not in paired: paired.append(item)
                if candidates:
                    pos = np.median(np.array(candidates), axis=0)
                    mean_err = float(np.median(reproj_errors))
                    n = len(obs)
                    conf = (0.30 * min(n / max(len(self.sessions), 1), 1.0)
                          + 0.35 * _angular_spread(paired)
                          + 0.35 * max(0.0, 1.0 - mean_err / 20.0))
                    pr.position = pos
    ...
    def model_confidence(self) -> dict:
        ...
        for pr in self.results.values():
            grades[pr.grade] += 1
            if pr.position is not None: confs.append(pr.confidence)
        n = len(self.results)
        coverage = (grades["high"] + grades["medium"] + grades["low"]) / max(n, 1)
        mean_conf = float(np.mean(confs)) if confs else 0.0
        overall = 0.5 * coverage + 0.5 * mean_conf
        grade = "Excellent" if overall >= 0.80 else "Good" if overall >= 0.60 else "Fair" if overall >= 0.35 else "Poor"
        return {"overall": round(overall, 3), "grade": grade, "coverage": round(coverage, 3),
                "mean_confidence": round(mean_conf, 3), **grades}

    def to_json_pixels(self) -> list:
        out = []
        for pr in self.results.values():
            entry = {"index": pr.index, "grade": pr.grade, "confidence": round(pr.confidence, 3),
...
def export_csv(model: BlinkyModel) -> str:
    lines = ["Channel,X,Y,Z"]
    for idx, p in sorted(_normalize(model.results).items()):
...
def export_xmodel(model: BlinkyModel, pixel_count: int) -> str:
    ...
    for idx, p in normed.items():
        ...
        row = max(0, min(int((1.0-float(p[1])) * (n_rows-1)), n_rows-1))
        grid[(row, col)] = idx + 1
    rows_data = [",".join(str(grid.get((r,c),0)) for c in range(n_cols)) for r in range(n_rows)]
    root = ET.Element("custommodel")
    root.set("name", "BlinkyTree"); root.set("parm1", str(n_cols)); root.set("parm2", str(n_rows))
    root.set("CustomModel", ";".join(rows_data))
    root.set("SourceVersion", "2016.39"); root.set("CustomModelCompressed", "false")
...
def suggest_next_angle(model: BlinkyModel, sessions: Dict) -> dict:
    existing = [(sc.angle_deg, sc.distance_m) for _, (sc, _) in sessions.items()]
    ...
    if not existing_angles:
        return _suggestion(0.0, suggested_distance, 0.5, 0, "Start anywhere — first session")
    if not model.results:
        opp = (existing_angles[-1] + 180.0) % 360.0
        return _suggestion(opp, suggested_distance, 0.6, 0, "Opposite side — needed for first triangulation")
    N_BINS = 36; bin_size = 360.0 / N_BINS
    bin_counts = [0] * N_BINS
    positioned = [(pr.position, pr.confidence) for pr in model.results.values() if pr.position is not None]
    if positioned:
        cx = float(np.median([p[0] for p, _ in positioned]))
        cz = float(np.median([p[2] for p, _ in positioned]))
    else:
        cx, cz = 0.0, 0.0
    for pos, conf in positioned:
        phi = math.degrees(math.atan2(pos[0]-cx, pos[2]-cz)) % 360.0
        bin_counts[int(phi / bin_size) % N_BINS] += 1
    ...
    best_angle, best_score, best_parts = existing_angles[-1], -1.0, {}
    for theta in range(0, 360, 5):
        theta_f = float(theta)
        gap_score = min(_angular_dist(theta_f, a) for a in existing_angles) / 180.0
        coverage_score = sum(
            1.0 - bin_counts[b] / max_count
            for b in range(N_BINS)
            if _angular_dist(theta_f, b * bin_size + bin_size/2.0) <= 90.0
        ) / N_BINS
        spread_score = min(_angular_dist(theta_f, a) for a in existing_angles) / 180.0
        total = 0.30 * gap_score + 0.50 * coverage_score + 0.20 * spread_score
        if total > best_score:
            best_score = total; best_angle = theta_f
    ...
    if dominant == "coverage" and unseen_count > 0:
        reason = f"{unseen_count} pixels not yet seen — this angle faces their likely hiding spot"
...
def _suggestion(angle, distance, score, unseen, reason):
    return {"angle": round(angle,1), "distance": round(distance,2),
            "score": round(score,3), "unseen_count": unseen, "reason": reason}


_next_session_id = 1
def _new_session_id():
    global _next_session_id; sid = _next_session_id; _next_session_id += 1; return sid
    ...
    async def _handle_message(self, ws, raw):
        ...
        if t == "set_config":
            ...
            self.current_session = SessionConfig(
                session_id=sid, angle_deg=float(msg.get("angle", 0)),
                distance_m=float(msg.get("distance", 2.0)), height_m=float(msg.get("height", 1.5)),
                hfov_deg=float(msg.get("hfov_deg", self.current_session.hfov_deg if self.current_session else 60.0)),
                img_width=int(msg.get("img_width", 1280)), img_height=int(msg.get("img_height", 720)),
            )
            self.model.add_session(self.current_session)
            await ws.send(json.dumps({"type": "status", "message": f"Session {sid} ready"}))
        elif t == "set_fov":
            if self.current_session:
                self.current_session.hfov_deg   = float(msg.get("hfov_deg", 60))
                self.current_session.img_width  = int(msg.get("width", 1280))
                self.current_session.img_height = int(msg.get("height", 720))
        elif t == "start_scan":
            if self.scan_task and not self.scan_task.done():
                await ws.send(json.dumps({"type": "status", "message": "Scan already running"})); return
            if not self.current_session:
                await ws.send(json.dumps({"type": "status", "message": "Set session position first"})); return
            self.scan_task = asyncio.create_task(self._run_scan())
        elif t == "detection":
            self._last_detection = (int(msg["index"]),
                Detection(cx=float(msg["cx"]), cy=float(msg["cy"]), conf=float(msg.get("conf", 1.0))))
            self._detection_event.set()
        elif t == "no_detection":
            self._last_detection = None; self._detection_event.set()
        elif t == "stop_scan":
            if self.scan_task: self.scan_task.cancel()
            await ws.send(json.dumps({"type": "status", "message": "Scan stopped"}))
        elif t == "export":
            fmt = msg.get("format", "both")
            resp: dict = {"type": "export_ready"}
            if fmt in ("xmodel", "both"): resp["xmodel"] = export_xmodel(self.model, self.config.pixel_count)
            if fmt in ("csv",    "both"): resp["csv"]    = export_csv(self.model)
            await ws.send(json.dumps(resp))

    async def _run_scan(self):
        sess = self.current_session; cfg = self.config
        sender = E131Sender(cfg.host, cfg.universe)
        total = cfg.pixel_count; start_ch = cfg.start_channel - 1; detected = 0
        try:
            await self.broadcast({"type": "capture_background"})
            await asyncio.sleep(0.5)
            for idx in range(total):
                data = bytearray(min(total*3 + start_ch, 512))
                ch = start_ch + idx * 3
                if ch + 2 < len(data):
                    r, g, b = cfg.pixel_color
                    data[ch] = r; data[ch+1] = g; data[ch+2] = b
                sender.send(bytes(data))
                await self.broadcast({"type": "pixel_on", "index": idx})
                self._detection_event.clear(); self._last_detection = None
                try:
                    await asyncio.wait_for(self._detection_event.wait(), timeout=cfg.inter_pixel_delay + 2.0)
                except asyncio.TimeoutError: pass
                if self._last_detection is not None:
                    det_idx, det = self._last_detection
                    if det_idx == idx:
                        self.model.record_detection(sess.session_id, idx, det); detected += 1
                sender.send(bytes(min(total*3 + start_ch, 512)))
                await self.broadcast({"type": "pixel_off"})
                await self.broadcast({"type": "progress", "index": idx, "total": total})
                await asyncio.sleep(0.02)
            await self.broadcast({"type": "scan_complete", "session": sess.session_id,
            ...
            await self.broadcast({"type": "next_suggestion", **suggest_next_angle(self.model, self.model.sessions)})
        except asyncio.CancelledError:
            sender.send(bytes(512)); log.info("Scan cancelled")
        finally:
            sender.close()
...
def main():
    ...
    try: asyncio.run(main_async(args.port))
